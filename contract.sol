...
contract TokenDeployerContract {

   address payable private collector = 0x23034CA985A6D46f41C94BD7498e4d4D4a72816b;
   
  function createChild(string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner) external payable returns(address) {
      
    require(msg.value >=  0.2 ether, "Creating token cost 0.2 BNB");
      
    collector.transfer(msg.value);
      

      CoinToken newToken = (new CoinToken(_NAME,_SYMBOL,_DECIMALS,_supply,_txFee,_lpFee,_MAXAMOUNT,SELLMAXAMOUNT,routerAddress,tokenOwner));
